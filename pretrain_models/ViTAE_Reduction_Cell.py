...
import numpy as np
from .ViTAE_token_transformer import Token_transformer
from .ViTAE_token_performer import Token_performer
...
class msvit(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, token_dims=64, downsample_ratios=4, kernel_size=7,
        ...
        for _ in range(3):
            PLFEStride.append((residual > 0) + 1)
            residual = residual // 2
        ...
        if tokens_type == 'none':
            ...
            self.MSCE = PatchEmbed(img_size, patch_size, in_chans, token_dims)
            num_patches = self.MSCE.num_patches
        ...
        if tokens_type not in ['none', 'embedding']:
            self.PLFE = nn.Sequential(
                            nn.Conv2d(in_chans, embed_dims, kernel_size=(3, 3), stride=PLFEStride[0], padding=(1, 1), groups=group),  # the 1st convolution
                            nn.SiLU(inplace=True),
                            nn.Conv2d(embed_dims, embed_dims, kernel_size=(3, 3), stride=PLFEStride[1], padding=(1, 1), groups=group),  # the 1st convolution
                            nn.BatchNorm2d(embed_dims),
                            nn.SiLU(inplace=True),
                            nn.Conv2d(embed_dims, token_dims, kernel_size=(3, 3), stride=PLFEStride[2], padding=(1, 1), groups=group),  # the 1st convolution
                            nn.SiLU(inplace=True))

            self.MSCE = MSCE(img_size=img_size, kernel_size=kernel_size, downsample_ratio=downsample_ratios, dilations=self.dilations,
                in_chans=in_chans, embed_dim=embed_dims, share_weights=share_weights, op=op)
            self.outSize = self.outSize // downsample_ratios

            in_chans = self.MSCE.out_chans
    ...
    def adjust_to_perfect_square(self, tensor, fill_value=0):
        B, N, C = tensor.shape
        target_n = int(math.sqrt(N))
        target_N = target_n * target_n

        if target_N == N:
            return tensor  # 已经是完全平方数，无需调整

        if target_N < N:
            # 需要裁剪
            max_index = target_n * target_n
            return tensor[:, :max_index, :]
        else:
            # 需要填充
            pad_size = target_N - N
            padding = torch.full((B, pad_size, C), fill_value, dtype=tensor.dtype, device=tensor.device)
            return torch.cat([tensor, padding], dim=1)
    def forward(self, x):
        
        if self.MSCE is None:
            return x
        ...
        if len(x.shape) < 4:
            ...
            # 调整张量尺寸
            x = self.adjust_to_perfect_square(x) # 调整张量尺寸确保其为完全平方数
            B, N, C = x.shape
            n = int(math.sqrt(N))
            x = x.view(B, n, n, C).contiguous()
            x = x.permute(0, 3, 1, 2)
        ...
        if self.pos_embed is not None:
            ...
            MSCE_x = torch.cat((cls_tokens, MSCE_x), dim=1)
            MSCE_x = MSCE_x + self.pos_embed

        if self.attn is None:
            return MSCE_x

        convX = self.PLFE(x)
        x = self.attn.attn(self.attn.norm1(MSCE_x))

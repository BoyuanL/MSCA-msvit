...
class Softmax(nn.Module):
    ...
    def forward(self, input, label, x=None):
...
class EnhancedAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.ms_projection = nn.Linear(dim, inner_dim, bias=False)
    ...
    def forward(self, x, ms_features=None, mask=None):
        # 标准自注意力机制
        qkv = self.to_qkv(x).chunk(3, dim=-1)
        ...
        if ms_features is not None:
            # 确保维度匹配
            if ms_features.shape[1] != x.shape[1]:
                # 如果特征序列长度不同，进行调整（例如插值或裁剪）
                ms_features = F.interpolate(
                    ms_features.permute(0, 2, 1), 
                    size=x.shape[1],
                    mode='linear'
                ).permute(0, 2, 1)
            
            # 投影多尺度特征
            ms_v = self.ms_projection(ms_features)
            ms_v = rearrange(ms_v, 'b n (h d) -> b h n d', h=self.heads)
            
            # 加权融合原始特征和多尺度特征
            fusion_weight = torch.sigmoid(self.alpha)
            out = fusion_weight * out + (1 - fusion_weight) * ms_v
        
        out = rearrange(out, 'b h n d -> b n (h d)')
        return self.to_out(out)

# 修改后的Transformer Block
class EnhancedBlock(nn.Module):
    def __init__(self, dim, heads, dim_head, mlp_dim, dropout=0.):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = EnhancedAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout)
    ...
    def forward(self, x, ms_features=None, mask=None):
        x = x + self.attn(self.norm1(x), ms_features, mask)
...
class EnhancedTransformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(EnhancedBlock(dim, heads, dim_head, mlp_dim, dropout))

    def forward(self, x, ms_features=None, mask=None):
        for block in self.layers:
            x = block(x, ms_features, mask)
        return x

# 修改后的ViT_face，支持多尺度特征融合
class EnhancedViT_face(nn.Module):
    def __init__(self, *, loss_type, GPU_ID, num_class, image_size, patch_size, dim, depth, heads, mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.):
        ...
        # 使用增强版Transformer
        self.transformer = EnhancedTransformer(dim, depth, heads, dim_head, mlp_dim, dropout)
    ...
    def forward(self, img, ms_features=None, label=None, mask=None):
        p = self.patch_size
        ...
        # 将多尺度特征传递给transformer
        x = self.transformer(x, ms_features, mask)

        x = x.mean(dim=1) if self.pool == 'mean' else x[:, 0]

        x = self.to_latent(x)
        emb = self.mlp_head(x)
        if label is not None and self.loss is not None:

...
import numpy as np
from .ViTAE_Reduction_Cell import ReductionCell
from .ViTAE_Normal_Cell import NormalCell
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        x = self.RC(x)  # 25,3,64,64 ---> 25,256,64 ---> 25,64,64
        for nc in self.NC:
            x = nc(x)
        return x

class ViTAE_face(nn.Module):
